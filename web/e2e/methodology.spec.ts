...
test("methodology page includes estimate language and the formula", async ({
  ...
  await expect(
    page.getByText("p = 1 - Phi((130 - mu) / sigma)").first(),
  ).toBeVisible();
...
test("tail calculator updates the modeled estimate from μ/σ", async ({
  ...
  await page.goto("/methodology/");
  const mu = page.getByRole("spinbutton", { name: "Mean μ (IQ points)" });
  await expect(mu).toHaveValue("100");
  await expect(
    page.getByText("Modeled estimate: 2.28%", { exact: false }),
  ).toBeVisible();

  await mu.fill("90");
  await expect(page.getByText("Modeled estimate: 0.38%")).toBeVisible();
});
